	...
	<template>
		...
		<div :class="['form-row', rowId]">
			...
			<input
				:name="id"
				:id="inputId"
				v-model="value[modelIsObject ? propId : 'val']"
				v-bind="inputAttrs"
				@click="openSelector"
			...
			<!-- SELECTOR -->
			<popup ref="selector" class="date-selector" :aria-label="t('Fenster zum Editieren des Datums')">
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
	...
	<script setup>
		...
		*/
		...
		const { modelIsObject, isMultiLang, value, rowId, inputId, propId, showError, updateInput, removeError, labelAttrs } = useInput(props, emit);
		...
		const currentValue = computed(() => props.modelValue ? (modelIsObject.value?value.value[propId]:value.value.val) : null);
		const currentValueDate = computed(() => currentValue.value ? currentValue.value.split(" ")[0] : null);
		...
		function confirm() {
			...
			// update model
			if(modelIsObject.value) { value.value[propId] = val; }
			else { value.value.val = val; }
			updateInput();
	...
	<i18n lang="json5">
		{
			...
			"en": {
				...
				"Fenster zum Editieren des Datums": "Popup for editing date",
